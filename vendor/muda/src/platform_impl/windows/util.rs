...
pub fn decode_wide(w_str: *mut u16) -> String {
    // Avoid enabling windows-sys' whole Globalization module just for this
    // null-terminated UTF-16 length query. In windows-sys 0.61 that module
    // also contributes raw ICU imports, which are incompatible with Slint's
    // bundled ICU when lld-link resolves the Windows executable.
    let len = unsafe {
        let mut len = 0usize;
