...
/// JNI is an application boundary: a Rust panic must become a normal error
/// result rather than unwinding through Java frames and aborting the Android
/// process. The closure is deliberately `AssertUnwindSafe`; JNI handles and
/// registry guards never escape the closure, and every registry lock is
/// released before any potentially blocking engine operation.
fn catch_native<T, F>(operation: F) -> Result<T, String>
where
...
/// Convert a caught operation result into a Java string without allowing a
/// second formatting/allocation panic to cross the JNI boundary.
fn json_response(
    env: &mut JNIEnv<'_>,
